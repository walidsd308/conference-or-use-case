...
The problem addresses the medium-term scheduling of elective orthopedic surgery patients at the University Hospital of Montpellier. This task generates an optimized schedule by assigning patients ($p \in \mathcal{P}$) to specific days ($d \in \mathcal{D}$), operating rooms ($b \in \mathcal{B}$), and physicians ($h \in \mathcal{H}$), within the planning horizon $T$ (see Table \ref{tab:parameters} for notation).
