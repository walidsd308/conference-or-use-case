...
The scheduling operates within a clinical workflow that differs from traditional batch scheduling approaches. In practice, patients are not accumulated and scheduled in large batches at fixed weekly intervals. Instead, the system processes patients incrementally as they complete their consultation appointments, where the operating physician determines the surgical need and assigns the patient to their surgical schedule.
...
\textbf{Hierarchical Objectives:} The objective minimizes a weighted sum of violations, structured hierarchically by penalty weights ($\alpha_1$ to $\alpha_5$):
    \begin{itemize}
        ...
        \item \textbf{Priority 3:} Minimize specialty fragmentation ($\alpha_4$) and efficiently manage day usage ($\alpha_5$), which promotes operational stability and maximizes resource utilization.
    \end{itemize}
...
This heuristic prioritizes patients with the most flexible schedules (high availability scores) based on the MILP results for early selection. Such flexible patients can choose from multiple good-quality slots. For each unscheduled patient pp
p, an availability score $\sigma_p$ quantifies scheduling flexibility:
...
Patients with higher availability scores $\sigma_p$ are prioritized in the selection sequence. After each patient confirms a selection, slot availability is updated and scores are recomputed for remaining patients. Algorithm \ref{alg:patient_ordering}
