...
Patients select their surgery time slots sequentially. Once a patient confirms a slot, it is fixed and the remaining proposed slots are adapted accordingly: constraints are re-checked, capacity is updated, and conflicts are resolved. The order in which patients are presented therefore affects solution quality. Unlike greedy or priority-based sequencing, this heuristic adapts the MILP solution after each confirmation rather than assigning from a static list; this bounds computation at the cost of global optimality.

The heuristic presents the most flexible patient first. For each unscheduled patient $p$, an availability score $\sigma_p$ quantifies scheduling flexibility:
...
Patients with higher $\sigma_p$ are selected first, as detailed in Algorithm \ref{alg:patient_ordering}. For each patient $p$, the set of proposed slots $\mathcal{S}_p$ is derived from the MILP solution: each slot corresponds to a $(d,b,h)$ triple where $X_{d,b,h,p,i}=1$. Confirming a slot $s^* \in \mathcal{S}_{p^*}$ fixes the corresponding $X$ variables; the remaining slots are then validated against updated constraints, and scores are recomputed for all remaining patients.

At each iteration, the update step validates remaining slots against updated constraints: the assigned physician must be available at the candidate block and day, the patient must not already hold a confirmed slot on that day, the scheduling day must fall within the patient's feasibility window, and the residual block capacity must be sufficient for the intervention. Block capacity is decremented by $(\tau_p + \tau^{\text{post}}_p)$ after each confirmed assignment, so that feasibility checks reflect current resource usage. Each patient carries a reprogramming type (\emph{before} or \emph{after}) that determines whether scheduling outside $[d^{\text{arr}}_p, d^{\text{dep}}_p]$ is permitted, restricting the set $\mathcal{S}_p$ accordingly. When a selected slot shares its block-day with another patient's proposed slot, the system detects the conflict and flags the displaced patient.
...
\While{$\mathcal{P}^{\text{remain}} \neq \emptyset$}
    ...
    \State Adapt: Update $\mathcal{S}_p$ for all $p \in \mathcal{P}^{\text{remain}}$ given fixed assignments $A^*$
\EndWhile
\State \Return $A^*$
\end{algorithmic}
\end{algorithm}
