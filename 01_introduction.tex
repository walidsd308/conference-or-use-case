...
This work balances operational efficiency with patient-centered care, enabling patients to actively participate in the scheduling process.
...
This patient-centered approach is implemented through an iterative optimization methodology, illustrated in Figure \ref{fig:methodology_flow}. The MILP model generates up to three distinct feasible time slots for each patient, with each option satisfying all clinical and operational constraints. The Flexibility-First heuristic identifies the patient with the highest scheduling flexibility, measured by availability score $\sigma_p$ (detailed in Section \ref{sec:patient_ordering}). The selected patient reviews the available options and confirms one preferred event. The system then redistributes unused events to remaining patients and recalculates the next patient to process. This approach transforms patients from passive recipients of scheduling decisions into active participants in the planning process, demonstrating that patient autonomy and operational efficiency are complementary objectives.
